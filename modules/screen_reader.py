            ...
            for category_key, category_name, category_desc in categories:
                ...
                if elements:
                    ...
                    if category_key == "links":
                        # Analyse parallèle des liens
                        self.logger.info("Analyse parallèle des liens en cours...")
                        results = self._analyze_links_parallel(elements, batch_size=5)  # Réduire la taille des lots
                        self.non_conformites["liens"].extend(results)
                        
                        # Analyse des autres attributs des liens en parallèle
                        self.logger.info("Analyse des attributs supplémentaires des liens...")
                        with ThreadPoolExecutor(max_workers=8) as executor:
                            futures = []
                            for element in elements:
                                futures.append(executor.submit(self._print_element_table, element, category_name))
